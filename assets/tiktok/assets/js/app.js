...
async function loadData(){
  ...
  categorias = c;
  ideas = JSON.parse(localStorage.getItem("allpa_ideas_override") || JSON.stringify(i));
  metricas = JSON.parse(localStorage.getItem("allpa_metricas_override") || JSON.stringify(m));
  aprendizajes = JSON.parse(localStorage.getItem("allpa_aprendizajes_override") || JSON.stringify(a));
  initFilters();
  render();
}
...
function saveLocal(){ 
  localStorage.setItem("allpa_ideas_override", JSON.stringify(ideas)); 
  localStorage.setItem("allpa_metricas_override", JSON.stringify(metricas)); 
  localStorage.setItem("allpa_aprendizajes_override", JSON.stringify(aprendizajes)); 
  updateKpis(); 
}
function filtered(){
...
function strategy(){
 ...
 return `<h2>Mapa estratégico</h2><p>Cada video debe unir deseo del productor, historia de Stiven/Allpa, producto y principio de persuasión.</p>
...
function detail(id){
 ...
 <label>Notas</label><textarea onchange="updateIdea(${i.id},'notas',this.value)">${i.notas||""}</textarea>
 <div class="scriptbox"><b>Estructura de guion:</b><br>1. Hook fuerte.<br>2. Dolor del productor.<br>3. Historia o demostración.<br>4. Mecanismo Allpa.<br>5. Resultado soñado.<br>6. CTA a WhatsApp.</div>
...
function updateKpis(){
 ...
 kpiHigh.textContent=metricas.filter(i=>i.senal_venta==="Alta").length;
...
function exportData(){
 ...
 const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="allpa-video-tracking-export.json"; a.click();
}
...
function resetLocal(){ localStorage.removeItem("allpa_ideas_override"); localStorage.removeItem("allpa_metricas_override"); localStorage.removeItem("allpa_aprendizajes_override"); location.reload(); }
loadData();
